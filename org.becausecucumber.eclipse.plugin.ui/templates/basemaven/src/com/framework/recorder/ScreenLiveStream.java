 ...
 */
package com.framework.recorder;
...
public class ScreenLiveStream extends TimerTask {
	...
	public ScreenLiveStream(RecordConfig recordConfig,
			SequenceEncoder sequenceEncoder) throws IOException {
	...
	public void run() {
		...
		if (useCursor || useWatermark) {
			...
			if (useWatermark) {
				// draw watermark
				imageGraphics.drawImage(watermarkImage, watermarkLocationX,
						watermarkLocationY, null);
			}
			...
			if (useCursor) {
				...
				// draw cursor
				imageGraphics.drawImage(cursorImage, cursorLocation.x,
		...
		try {
			...
			// Logger.getLogger(ScreenLiveStream.class.getName()).log(Level.SEVERE,
			// null, ex);
		}
	}
}
