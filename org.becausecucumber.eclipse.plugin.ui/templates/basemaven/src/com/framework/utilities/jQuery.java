...
public class jQuery implements Iterable<WebElement> {
	...
	private void check() {
		if (closed) {
			throw new IllegalStateException(
					"Cannot use a closed jquery object.");
		}
	}
	...
	public jQuery clone(boolean with_data_and_events, boolean deep) {
		return subset(".clone(arguments[0],arguments[1])",
				with_data_and_events, deep);
	}
	...
	public int combinedAttrHash(String attr) {
		return ((String) js("var v = '';" + "var attr = arguments[0];" + ref
				+ ".each(function(){v+='|'+" + jqf.getRef()
				+ "(this).attr(attr);});" + "return v", attr)).hashCode();
	}

	public int combinedMethodHash(String meth) {
		return ((String) js("var v = '';" + "var meth = arguments[0];" + ref
				+ ".each(function(){v+='|'+" + jqf.getRef()
				+ "(this)[meth]();});" + "return v", meth)).hashCode();
	}
	...
	/**
	 * Returns an integer indicating the position of the original element
	 * relative to the elements matched by the selector. If the element is not
	 * found, .index() will return -1.
	 * 
	 * @see http://api.jquery.com/index/
	 * @return
	 */
	...
	public boolean isTimeout() {
		if (timeout == 0) {
			throw new IllegalArgumentException(
					"Cannot tell if timed out -- no timeout set");
		}
	...
	public Position offset() {
		...
		List<Long> offset = (List<Long>) js("var offset = " + ref
				+ ".offset();" + "return [offset.top,offset.left];");
	...
	public Position offsetParent() {
		@SuppressWarnings("unchecked")
		List<Long> offset = (List<Long>) js("var offset = " + ref
				+ ".offsetParent();" + "return [offset.top,offset.left];");
		return new Position(offset.get(0), offset.get(1));
	}
	...
	public jQuery parentsUntil(String selector, String filter) {
		return subset(".parentsUntil(arguments[0],arguments[1])", selector,
				filter);
	}
	...
	public jQuery pause(String msg) throws TimeoutException {
		try {
			if (isTimeout()) {
				...
				clearTimeout();
				throw new TimeoutException("Timeout " + toString() + " after "
						+ timeout_old + " ms:" + msg);
			}
			Thread.sleep(200);
	...
	public Position position() {
		@SuppressWarnings("unchecked")
		List<Long> position = (List<Long>) js("var position = " + ref
				+ ".position();" + "return [position.top,position.left];");
	...
	public jQuery refresh() {
		@SuppressWarnings("unchecked")
		List<Object> refresh = (List<Object>) js("return [" + ref + ".length,"
				+ ref + ".selector]");
	...
	public jQuery refreshUntil(int min, int max, long timeout)
		...
		while (true) {
			...
			if (System.currentTimeMillis() > to) {
				throw new TimeoutException("Looking for " + selector + " with "
						+ min + " to " + max + " results.");
			}
	...
	public jQuery refreshUntilAtLeast(int min, long timeout)
			throws TimeoutException {
	...
	public jQuery refreshUntilAtMost(int max, long timeout)
			throws TimeoutException {
	...
	public jQuery untilChanged(long timeout) throws TimeoutException {
		if (html_hash == -1) {
			throw new IllegalArgumentException(
					"Cannot wait until changed if html has never been marked (mark())");
		}
		setTimeout(timeout);
		try {
			while (true) {
	...
	public int untilCombinedAtterHashChanged(int previous_hash, String attr,
	...
	public int untilCombinedAttrHashChanged(int previous_hash, String attr)
			throws TimeoutException {
		return untilCombinedAtterHashChanged(previous_hash, attr,
				jqf.getDefaultTimeout());
	}

	public int untilCombinedMethodHashChanged(int previous_hash, String method)
			throws TimeoutException {
		return untilCombinedMethodHashChanged(previous_hash, method,
				jqf.getDefaultTimeout());
	}
