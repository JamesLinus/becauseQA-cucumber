...
import com.framework.utilities.TimeUtils;
...
public class TestRailAPI {
	...
	public TestRailAPI(String baseurl, String username, String password) {
		...
		client = new APIClient(getBase_Url());
		client.setUser(getUser());
		client.setPassword(password);
	}

	public TestRailAPI() {
		this.setBase_Url("https://gdcqatestrail01/testrail");
		;
	...
	public boolean getProjectByName(String projectname)
		...
		if (sendGet instanceof JSONArray) {
			...
			for (int k = 0; k < projects.size(); k++) {
				...
				if (foundproject.get("name").toString()
						.equalsIgnoreCase(projectname.trim())) {
	...
	public List<JSONObject> getPriorities() throws MalformedURLException,
			IOException {
	...
	public void getProjectConfiguration() throws MalformedURLException,
			IOException {
	...
	public boolean getUserID() throws MalformedURLException, IOException {
		...
		for (int k = 0; k < userinfo.size(); k++) {
			...
			if (singleuser.get("name").toString().toLowerCase()
					.contains(this.getUser().toLowerCase())
					|| singleuser.get("email").toString().toLowerCase()
							.contains(this.getUser().toLowerCase().trim())) {
	...
	public boolean getMilestone(String milestonename)
		...
		for (int k = 0; k < milestones.size(); k++) {
			...
			if (milestone.get("name").toString()
					.equalsIgnoreCase(milestonename.trim())
	...
	public boolean getLatestMilestone() throws MalformedURLException,
			IOException {
		boolean findmilestone = false;
		String url = String.format("get_milestones/%d", this.projectid);
		JSONArray milestones = (JSONArray) client.sendGet(url);
	...
	public long createMilestone(String milestonename, String description,
			String duedate) throws MalformedURLException, IOException {
		...
		try {
			finalseconds = new SimpleDateFormat("yyyy-MM-dd").parse(duedate)
					.getTime();
		...
		JSONObject milestone = (JSONObject) client.sendPost(newmilestone,
				postData);
		this.milestoneid = (Long) milestone.get("id");
		// return true;
	...
	public long updateMilestone(String milestonename, String description,
			String duedate) throws MalformedURLException, IOException {

		String newmilestone = String.format("update_milestone/%d",
				this.milestoneid);
		HashMap<Object, Object> postData = new HashMap<Object, Object>();
		postData.put("name", milestonename);
		postData.put("description", description);
	...
	public boolean getTestPlan(String planname) throws MalformedURLException,
			IOException {
	...
	public void getTestPlan(long planid) throws MalformedURLException,
			IOException {
	...
	public boolean getPlanEntryName(String entryname)
		...
		for (int k = 0; k < entries.size(); k++) {
			...
			for (int j = 0; j < runarray.size(); j++) {
				JSONObject run = (JSONObject) runarray.get(j);
				String findentryname = (String) run.get("name");
	...
	public boolean getPlanEntry(long suiteid) throws MalformedURLException,
			IOException {
		boolean iscreated = false;
	...
	public boolean addPlanEntry(long suiteid) throws MalformedURLException,
			IOException {
		boolean iscreated = false;
	...
	public boolean addPlanEntryforName(long suiteid, String name,
			List<Integer> configs) throws MalformedURLException, IOException {
		boolean iscreated = false;
		// String testSuite = getTestSuite(suiteid);
		String url = String.format("add_plan_entry/%s", this.planid);
	...
	public boolean updatePlanEntryForCase(long suiteid, List<Long> caselist)
		...
		String url = String.format("update_plan_entry/%s/%s", this.planid,
				this.entryid);
	...
	public long createTestPlan(String planname) throws MalformedURLException,
			IOException {
	...
	public long updateTestPlan(String planname) throws MalformedURLException,
			IOException {
	...
	public void closeTestPlan(String planname) throws MalformedURLException,
			IOException {
		String url = String.format("get_plans/%d", this.projectid);
		JSONArray plans = (JSONArray) client.sendGet(url);
		/*
		 * this.planid=63933; this.milestoneid=825;
		 * updateTestPlan("jenkins job sample-Automation-333"); String
		 * closeurl2=String.format("delete_plan/%d", 63933);
		...
		for (int k = 0; k < plans.size(); k++) {
			...
			if (plan.get("name").toString().toLowerCase()
					.contains(planname.toLowerCase().trim())) {
				// &&((Boolean)plan.get("is_completed"))){
				String closeurl = String.format("close_plan/%s", plan.get("id")
						.toString());
	...
	public boolean getTestRun(String runame) throws MalformedURLException,
			IOException {
	...
	public long createTestPlanRunForSuite() throws MalformedURLException,
			IOException {
	 ...
	 * @Description: This function is a bug by now ,as metioned in offical
	 *               document ,so you should use the add_plan_entry intead it
	 *               http://forum.gurock.com/topic/1584/adding-a-new-test-run-
	 *               configuration-to-an-existing-entry/ it's a to-do list for
	 *               testrail team by 2014/8/3
	 * @deprecated
	 * @author alterhu2020@gmail.com
	 * @param @param runame
	...
	public boolean updateTestPlanRunForTestSection(String runame, long sectionid)
		...
		// getTestRun(runame);
		String runurl = String.format("update_plan_entry/%d/%s", this.planid,
				this.entryid);
		Map<Object, Object> arguments = new HashMap<Object, Object>();
		// arguments.put("suite_id", this.suiteid);
		arguments.put("name", runame);
	...
	public boolean updateTestPlanRunForCases(long[] caseids)
		...
		// getTestRun(runame);
		String runurl = String.format("update_plan_entry/%d/%d", this.planid,
				this.entryid);
		Map<Object, Object> arguments = new HashMap<Object, Object>();
		// arguments.put("suite_id", this.suiteid);
	...
	public boolean getTestSuite(String suitename) throws MalformedURLException,
			IOException {
		boolean iscreated = false;
	...
	public String getTestSuite(long suiteid) throws MalformedURLException,
			IOException {
	...
	public long createTestSuite(String suitename, String description)
		...
		JSONObject newsuite = (JSONObject) client.sendPost(newsuiteurl,
				postData);
	...
	public boolean getTestSection(String sectionname)
			throws MalformedURLException, IOException {
		boolean iscreated = false;
		String url = String.format("get_sections/%s&suite_id=%s",
				this.projectid, this.suiteid);
		JSONArray sections = (JSONArray) client.sendGet(url);
		for (int k = 0; k < sections.size(); k++) {
			JSONObject section = (JSONObject) sections.get(k);
			if (section.get("name").toString()
					.equalsIgnoreCase(sectionname.trim())) {
	...
	public boolean getDeepTestSection(String sectionname)
			throws MalformedURLException, IOException {
		boolean iscreated = false;
		String url = String.format("get_sections/%s&suite_id=%s",
				this.projectid, this.suiteid);
		...
		if (sectionname.contains("/")) {
			sectionlist = new LinkedList<String>(Arrays.asList(sectionname
					.split("/")));
			moresections = true;
		}
		...
		for (int k = 0; k < sections.size(); k++) {
			JSONObject section = (JSONObject) sections.get(k);
			/*
			 * if(section.get("name").equals(
			 * "ECash - Auth Commit confirmation notification")){
			...
			if (moresections) {
				for (int j = 0; j < sectionlist.size(); j++) {
					...
					if (c == null) {
						if (section.get("name").toString()
								.equalsIgnoreCase(sectionlist.get(j))) {
						...
						if (section.get("name").toString()
								.equalsIgnoreCase(sectionlist.get(j))
								&& section.get("parent_id").toString()
										.equals(tempparentid)) {
							tempparentid = section.get("id").toString();
							this.sectionid = (Long) section.get("id");
							sectionlist.remove(j);
	...
	public long createTestSection(String sectionname, long parentsectionid)
			throws MalformedURLException, IOException {
		...
		JSONObject newsection = (JSONObject) client.sendPost(newsectionurl,
				postData);
	...
	public long createRecusionTestSection(String sectionname)
			throws MalformedURLException, IOException {
		// parse the section string
	...
	public long updateTestSection(String sectionname)
			throws MalformedURLException, IOException {
		String newsectionurl = String.format("update_section/%d",
				this.sectionid);
		HashMap<String, Object> postData = new HashMap<String, Object>();
		postData.put("name", sectionname);
	...
	public long[] getCaseList(long sectionid) throws MalformedURLException,
			IOException {
		String url = String.format("get_cases/%s&suite_id=%s&section_id=%d",
				this.projectid, this.suiteid, sectionid);
	...
	public JSONArray getCaseFieldsList(long sectionid)
			throws MalformedURLException, IOException {
	...
	public boolean getCase(long caseid) throws MalformedURLException,
			IOException {
	...
	public long getSuiteidByCase(long caseid) throws MalformedURLException,
			IOException {
		long suiteid = 0;
		String url = String.format("get_case/%d", caseid);
		JSONObject caseobject = (JSONObject) client.sendGet(url);
	...
	public long getTestCase(String casename, long sectionid)
		...
		for (int caseindex = 0; caseindex < cases.size(); caseindex++) {
			JSONObject currentcase = (JSONObject) cases.get(caseindex);
			if (currentcase.get("title").toString()
					.equalsIgnoreCase(casename.trim())) {
	...
	public boolean updateTestCase(String caseName, long typeid,
			String estimate, long priorityid, long caseId, String refs,
		 ...
		 * postData.put("estimate", estimate); postData.put("priority_id",
		 * priorityid); // 4 means must test
		 */
		postData.put("custom_cucumber_written", true); // 4 means must test
		postData.put("custom_successfully_automated", true); // 4 means must
																// test
		postData.put("milestone_id", this.milestoneid); // Milestone attached to
														// case

		if (refs != null) // Add list of reference Id/requirement to test case
							// name if starts with project name
	...
	@SuppressWarnings("rawtypes")
	public long createTestCase(String caseName, long typeid, String estimate,
			long priorityid, String refs, HashMap parameters)
			throws MalformedURLException, IOException {
		long newcaseid = 0;
	...
	public boolean addTestResultForTestRun(long startseconds, long runid,
			long caseid, int status, String version, String rundescription)
			throws MalformedURLException, IOException {

		if (this.runid != 0 && this.userid != 0) {
			String url = String.format("add_result_for_case/%d/%d", runid,
					caseid);
			HashMap<Object, Object> postData = new HashMap<Object, Object>();
	...
	/*
	 * 
	 * public int covertResultStaus(String statusname){ int statuscode=3;
	 * //untested
	 * if(statusname.equalsIgnoreCase("passed")||statusname.equalsIgnoreCase
	 * ("passed")){ statuscode=1; //passed }else
	 * if(statusname.equalsIgnoreCase("fail"
	 * )||statusname.equalsIgnoreCase("failed")){ statuscode=5; //failed }else{
	...
	public void writeAutomationTestCase(String casedescription,
			String casetitle, String stepsdescription, String expectedresult,
			long typeid, String estimate, long priorityid, String refs,
			HashMap parameters) throws MalformedURLException, IOException {
		...
		if (caseid == 0) {
			System.out.println("we had not found this case:" + casetitle
					+ ",so we will create a new one");
			createTestCase(casetitle, typeid, estimate, priorityid, refs,
					parameters);
		} else {
			System.out.println("we found this case created before:" + casetitle
					+ ",so we just update this case");
			updateTestCase(casetitle, typeid, estimate, priorityid, caseid,
					refs, parameters);
		}
