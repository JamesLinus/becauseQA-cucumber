...
public class LicenseHandling {
	...
	private static class DefaultLicenseUserInteractionHandler implements
		...
		public XFormDialog createDialog() {
			...
			//
			// ((XFormDialog)
			// localObject).getFormField("###Choose license type")
		...
		public void showNewLicenseInfo(String paramString) {
			...
			String str = "<html><body><div>Name: "
					+ LicenseHandling.e.getName() + "<br>Organization: " + org
					+ "<br>E-Mail: " + LicenseHandling.e.getEmail()
	...
	static class ProxyBackup {
		...
		public void backup() {
			...
			//
			// for (Object localObject1 = a.iterator(); ((Iterator)
			// localObject1)
			// .hasNext();) {
			...
			// }
			// if ((PlatformUtil.getCurrentPlattform() !=
			// PlatformUtil.Platform.WIN)
			// && (localObject1 == null)) {
			// try {
			// (localObject2 = new ProxySearch())
		...
		public void restore() {
			// Properties localProperties = System.getProperties();
			//
			// for (Iterator localIterator = this.b.entrySet().iterator();
			// localIterator
			// .hasNext();) {
			// Map.Entry localEntry;
			// if ((localEntry = (Map.Entry) localIterator.next()).getValue() !=
			// null)
