...
public class MavenProjectWizardCustomArtifactPage extends AbstractMavenWizardPage {
	...
	// private static final ProjectFolder FILTERS = new
	// ProjectFolder("src/main/filters", null, false);

	// private static final ProjectFolder FILTERS_TEST = new
	// ProjectFolder("src/test/filters", null, false);

	// private static final ProjectFolder ASSEMBLY = new
	// ProjectFolder("src/main/assembly", null, false);

	// private static final ProjectFolder CONFIG = new
	// ProjectFolder("src/main/config", null, false);
	...
	static {
		...
		directorySets.put(MavenArtifactComponent.EAR, EAR_DIRS); // MNGECLIPSE-688
																	// add EAR
																	// Support
	}
	 ...
	 */
	public MavenProjectWizardCustomArtifactPage(ProjectImportConfiguration projectImportConfiguration) {
	 ...
	 * being complete as user input is required for continuing.
	 */
	protected MavenProjectWizardCustomArtifactPage(String name, ProjectImportConfiguration projectImportConfiguration) {
	...
	public void createControl(Composite parent) {
		...
		parentComponent.addBrowseButtonListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				MavenRepositorySearchDialog dialog = MavenRepositorySearchDialog.createSearchParentDialog(getShell(),
						org.eclipse.m2e.core.ui.internal.Messages.MavenProjectWizardArtifactPage_searchDialog_title,
						null, null);
		 ...
		 * { public void modifyText(ModifyEvent e) { validate(); } });
		 */
		...
		// addFieldWithHistory("selenium", artifactComponent.getSelenium());
		// //$NON-NLS-1$
	...
	public Model getModel() {
		...
		seleniumversion = artifactComponent.getSelenium();
		runbrowser = artifactComponent.getBrowser();
		runenv = artifactComponent.getEnvironment();
	...
	private String validateInput() {
		...
		if (artifactComponent.getSelenium().trim().length() == 0) {
			return "please specified the selenium version will be used in project";
		}
