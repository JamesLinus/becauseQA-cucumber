...
public class MewPomEdits {
	 ...
	 */
	public static void performOnDOMDocument(PomEdits.OperationTuple... fileOperations)
			throws IOException, CoreException {
		for (org.eclipse.m2e.core.ui.internal.editing.PomEdits.OperationTuple tuple : fileOperations) {
			IDOMModel domModel = null;
			// TODO we might want to attempt iterating opened editors and
			// somehow initialize those
			// that were not yet initialized. Then we could avoid saving a file
			// that is actually opened, but was never used so far (after
			// restart)
			try {
				...
				if (tuple.isReadOnly()) {
					domModel = (IDOMModel) StructuredModelManager.getModelManager()
							.getExistingModelForRead(tuple.getDocument());
					if (domModel == null) {
						domModel = (IDOMModel) StructuredModelManager.getModelManager()
								.getModelForRead((IStructuredDocument) tuple.getDocument());
					}
				} else {
					domModel = tuple.getModel() != null ? tuple.getModel()
							: (tuple.getFile() != null
									? (IDOMModel) StructuredModelManager.getModelManager()
											.getModelForEdit(tuple.getFile())
									: (IDOMModel) StructuredModelManager.getModelManager()
											.getExistingModelForEdit(tuple.getDocument())); // existing
																							// shall
																							// be
																							// ok
																							// here..
				...
				if (domModel != null) {
					...
					} else if (domModel.getId() != null) { // id will be null
															// for files outside
															// of workspace
