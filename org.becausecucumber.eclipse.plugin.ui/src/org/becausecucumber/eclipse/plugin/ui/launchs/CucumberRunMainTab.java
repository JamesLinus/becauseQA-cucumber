...
public class CucumberRunMainTab extends SharedJavaMainTab implements ILaunchConfigurationTab {
	...
	public void createRubyOptions(Composite comp) {
		...
		lblRubyArguments.setText("Ruby Arguments:");
	...
	public void performApply(ILaunchConfigurationWorkingCopy config) {
		...
		config.setAttribute(CucumberFeatureLaunchConstants.RUBY_PATH, rubypath.getText().trim());
		config.setAttribute(CucumberFeatureLaunchConstants.RUBY_PARAMETER, rubyparameter.getText().trim());
	...
	public void setDefaults(ILaunchConfigurationWorkingCopy config) {
		...
		String rubyparameter = "-EUTF-8 -e $stdout.sync=true;$stderr.sync=true;load($0=ARGV.shift)";

		System.out.println("Default path is:" + featurePath + "," + gluePath + "," + rubypath);

		if (javaProject != null && featurePath != null && rubypath != null) {
			initializeCucumberProject(isjava, gluePath, glueOptions, featurePath, rubypath, rubyparameter, javaProject,
					config);
		} else {
