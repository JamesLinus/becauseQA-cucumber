...
import org.becausecucumber.eclipse.plugin.common.searcher.priority.PriorityFunction;
import org.becausecucumber.eclipse.plugin.common.searcher.utils.JobUtil;
import org.becausecucumber.eclipse.plugin.common.searcher.utils.LineReader;
...
public class QuickTextSearcher {
	...
	private static List<IFile> stepfile;
	...
	private final class SearchInFilesWalker extends ResourceWalker {
		...
		protected void visit(IFile f, IProgressMonitor mon) {
			//System.out.println("Search File is:"+f.getName());
			if(f.getFileExtension()!=null){
				if(f.getFileExtension().equals("rb")||f.getFileExtension().equals("java")){
					stepfile.add(f);
				}
			}
			...
			try {
                ...
                /*    if(line.contains("I add a child page")&&query.getPatternString().contains("I add a child page")){
					   System.out.println("good status line is :"+line);
					   System.out.println("query step is:"+query.getPatternString());
					   System.out.println("compare is:"+query.matchItem(line));
                    }*/
					boolean found = query.matchItem(line);
					if (found) {
						LineItem lineItem = new LineItem(f, line.trim(), lineIndex, offset);
						add(lineItem);
					}
...
//		}
	...
	public  List<IFile> getStepfile() {
		return stepfile;
	}

	public  void setStepfile(List<IFile> stepfile) {
		QuickTextSearcher.stepfile = stepfile;
	}
