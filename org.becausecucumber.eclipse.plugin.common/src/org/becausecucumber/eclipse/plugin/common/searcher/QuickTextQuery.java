...
//				}
		...
		/*int rubystart=this.item.indexOf("^");
  	    int rubyend=this.item.indexOf("$");
  	    String regs="";
  	    if(rubystart>0&&rubyend>0){
  	       regs=item.substring(rubystart+1,rubyend);
  	    }
  	    this.pattern=Pattern.compile(regs, caseSensitive?0:Pattern.CASE_INSENSITIVE);*/
...
//	}
	...
	public boolean matchItem(String item) {
		...
		//this.item=item;
		//this.createMatcher(item,true);
		
		//Matcher matcher = pattern.matcher(this.orgPattern); //Creating garbage here
		return matcher.find();
	}
